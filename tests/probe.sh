...
check "rapidraw removed"            "pacman -Q rapidraw-bin >/dev/null 2>&1 || echo absent" "absent"
...
check "plymouth uses script module"  "grep ^ModuleName /usr/share/plymouth/themes/hycac/hycac.plymouth" "^ModuleName=script$"
check "tribar assets shipped"        "ls /usr/share/plymouth/themes/hycac/seg-red.png /usr/share/plymouth/themes/hycac/seg-track.png >/dev/null && echo yes" "yes"
check "greetd uses session wrapper"  "grep -c '^command.*hycac-niri-session' /etc/greetd/config.toml" "^1\$"
check "wrapper exec bit"             "test -x /usr/local/bin/hycac-niri-session && echo yes" "yes"
check "wrapper chatter in journal"   "sudo -n journalctl -t hycac-niri-session --no-pager -q | wc -l" "[1-9]"
check "user in input group"          "id -nG" "input"
check "evtest readable as user"      "bash -lc 'timeout 1 cat /dev/input/by-path/platform-i8042-serio-0-event-kbd >/dev/null 2>&1; echo rc=\$?'" "rc=(0|124)"
# bongocat's evtest reader only starts once the noctalia bar actually renders
# a widget; headless VMs (bochs-drm has no renderD128) never render, so only
# judge this on hardware that has a render node.
check "bongocat evtest spawned"      "bash -lc 'if [ ! -e /dev/dri/renderD128 ]; then echo vm-skip; else pgrep -u hycac -x evtest >/dev/null && echo yes || echo no; fi'" "yes|vm-skip"
